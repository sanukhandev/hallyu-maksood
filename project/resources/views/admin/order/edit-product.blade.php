              ...
              <div class="product-size">
                ...
                <ul class="siz-list">
                  @foreach(array_unique($productt->size) as $key => $data1)
                    <li class="{{ $item['size'] == $data1 ? 'active' : '' }}" data-key="{{ str_replace(' ','',$data1) }}">
                      <span class="box">
                        {{ $data1 }}     
                      </span>
                    </li>
              ...
              <div class="product-color">
                ...
                <ul class="color-list">
                  ...
                  @foreach($productt->color as $key => $data1)
                    ...
                    <li class="{{ ('#'.$item['color'] == $data1 && $productt->size[$key] == $productt->size[$item['size_key']]) ? 'active' : '' }} {{ $productt->IsSizeColor($productt->size[$key]) ? str_replace(' ','',$productt->size[$key]) : ''  }} {{ $productt->size[$key] == $productt->size[$item['size_key']] ? 'show-colors' : '' }}">
                        ...
                        <strong for="" class="text-capitalize">{{ str_replace("_", " ", $attrKey) }} :</strong>
                          ...
                          @foreach ($attrVal['values'] as $optionKey => $optionVal)
                            <div class="custom-control custom-radio">
                              ...
                              <input type="radio" id="{{$attrKey}}{{ $optionKey }}" name="{{ $attrKey }}" class="custom-control-input product-attr"  data-key="{{ $attrKey }}" data-price = "{{ $attrVal['prices'][$optionKey] * $curr->value }}" value="{{ $optionVal }}" {{ in_array($optionVal,(explode(",",$item['values'])))   ? 'checked' : '' }}>
