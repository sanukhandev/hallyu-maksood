...
class SubCategoryController extends VendorBaseController
{
    //*** JSON Request
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('status', function(Subcategory $data) {
                                ...
                                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('vendor-subcat-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>'.__("Activated").'</option><<option data-val="0" value="'. route('vendor-subcat-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.__("Deactivated").'</option>/select></div>';
                            })
                            ->addColumn('attributes', function(Subcategory $data) {
                                $buttons = '<div class="action-list"><a data-href="' . route('vendor-attr-createForSubcategory', $data->id) . '" class="attribute" data-toggle="modal" data-target="#attribute"> <i class="fas fa-edit"></i>'.__("Create").'</a>';
                                if ($data->attributes()->count() > 0) {
                                  $buttons .= '<a href="' . route('vendor-attr-manage', $data->id) .'?type=subcategory' . '" class="edit"> <i class="fas fa-edit"></i>'.__("Manage").'</a>';
                                }
                                $buttons .= '</div>';
